function DashboardCards({ requests }) {
  ...
  const pendingRequests = requests.filter((request) => request.status !== 'Closed' && request.status !== 'Archived').length;
  ...
  const cards = [
    { label: 'Total Requests', value: totalRequests, color: 'bg-blue-600' },
    { label: 'Pending Items', value: pendingRequests, color: 'bg-amber-500' },
    { label: 'Under Review', value: underReview, color: 'bg-purple-600' },
    { label: 'High Risk', value: highRisk, color: 'bg-rose-600' },
  ];
  ...
  return (
    <section>
      <div className="mb-6">
        ...
        <p className="text-slate-500 mt-1">Overview of Legal Affairs workload for Version 1.</p>
      </div>
      ...
      <div className="mt-6 bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
        ...
        <p className="text-slate-600">
          This dashboard uses mock frontend data. Later, the backend can provide live request counts,
          overdue requests, turnaround time, and department-based reports.
        </p>
